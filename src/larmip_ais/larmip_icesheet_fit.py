...
def larmip_fit_icesheet(
    pipeline_id,
    scaling_coefficients_dir,
):
    # Read in the ocean scaling
    NumOmodel = 19
    OS_NoDelay_R1, OS_WiDelay_R1, OS_Delay_R1 = ReadOceanScaling(
        os.path.join(scaling_coefficients_dir, "OceanScaling/OS_R1.dat")
    )
    OS_NoDelay_R2, OS_WiDelay_R2, OS_Delay_R2 = ReadOceanScaling(
        os.path.join(scaling_coefficients_dir, "OceanScaling/OS_R2.dat")
    )
    OS_NoDelay_R3, OS_WiDelay_R3, OS_Delay_R3 = ReadOceanScaling(
        os.path.join(scaling_coefficients_dir, "OceanScaling/OS_R3.dat")
    )
    OS_NoDelay_R4, OS_WiDelay_R4, OS_Delay_R4 = ReadOceanScaling(
        os.path.join(scaling_coefficients_dir, "OceanScaling/OS_R4.dat")
    )

    # Read melting sensitivity
    fname = os.path.join(
        scaling_coefficients_dir, "MeltSensitivity/MeltSensitivity.dat"
    )  # File to read
    with open(fname) as f:
    ...
    output = {
        "OS_NoDelay_R1": OS_NoDelay_R1,
        "OS_WiDelay_R1": OS_WiDelay_R1,
        "OS_Delay_R1": OS_Delay_R1,
        "OS_NoDelay_R2": OS_NoDelay_R2,
        "OS_WiDelay_R2": OS_WiDelay_R2,
        "OS_Delay_R2": OS_Delay_R2,
        "OS_NoDelay_R3": OS_NoDelay_R3,
        "OS_WiDelay_R3": OS_WiDelay_R3,
        "OS_Delay_R3": OS_Delay_R3,
        "OS_NoDelay_R4": OS_NoDelay_R4,
        "OS_WiDelay_R4": OS_WiDelay_R4,
        "OS_Delay_R4": OS_Delay_R4,
        "MeltSensitivity": MeltSensitivity,
        "NumOmodel": NumOmodel,
    }
...
def larmip_fit_smb(pipeline_id, preprocess_dict):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run the fit stage for the LARMIP ice sheet module",
