...
def larmip_project_icesheet(
    ...
    for model_idx, this_model in enumerate(models):
        ...
        RF_R1, RF_R2, RF_R3, RF_R4, RF_R5 = ReadResponseFunctions(
            model=this_model, rfunc_dir=rfunctions_dir, Tlen=Tlen
        )
    ...
    if cyear_start or cyear_end:
        for i in np.arange(nsamps):
            sl_r1[i, :] = ExtrapolateRate(
                sl_r1[i, :], targyears, cyear_start, cyear_end
            )
            sl_r2[i, :] = ExtrapolateRate(
                sl_r2[i, :], targyears, cyear_start, cyear_end
            )
            sl_r3[i, :] = ExtrapolateRate(
                sl_r3[i, :], targyears, cyear_start, cyear_end
            )
            sl_r4[i, :] = ExtrapolateRate(
                sl_r4[i, :], targyears, cyear_start, cyear_end
            )
            sl_r5[i, :] = ExtrapolateRate(
                sl_r5[i, :], targyears, cyear_start, cyear_end
            )
            sl_smb[i, :] = ExtrapolateRate(
                sl_smb[i, :], targyears, cyear_start, cyear_end
            )
    ...
    if ais_fpath:
        ...
        ais_ds.to_netcdf(
            ais_fpath,
            encoding={
                "sea_level_change": {"dtype": "float32", "zlib": True, "complevel": 4}
            },
        )
...
def make_ds(slr, region, targyears, baseyear, scenario, nsamps, pipeline_id):
    if region is None:
        ...
        description = "Global SLR contribution from Antarctica ({}) using the LARMIP module".format(
            region
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run the projection stage for the LARMIP ice sheet module",
    ...
    parser.add_argument(
        "--models",
        help="List of model names to include in the sampling process",
        nargs="+",
        default=None,
    )
    parser.add_argument(
